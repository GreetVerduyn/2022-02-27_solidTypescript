...
==== 3.I
I have created 3 iterfaces to login, each with their own methods (set token and check login).
The users class implements them all 3, the admin only implements the interface to login with a password.
...
If the  'Login as admin' checkbox is not checked, you are a guest and I only check the loging for guest.
If the box is checked, I check the loging for admin.
